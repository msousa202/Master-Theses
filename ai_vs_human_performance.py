...
for lvl in LEVELS:
    for crit in COLUMN_MAP[lvl]:
        ...
        t,p = ttest_rel(merged["Score_AI"], merged["Score_HU"], nan_policy="omit")
        # use Wilcoxon if sample < 10
        if merged.shape[0] < 10:
            w, p = wilcoxon(merged["Score_AI"], merged["Score_HU"])
            stat = w; test="Wilcoxon"
        else:
            stat = t; test="t"
        d = (merged["Score_AI"].mean() - merged["Score_HU"].mean()) / \
            merged[["Score_AI","Score_HU"]].stack().std()
        tests.append([lvl, crit, merged.shape[0], test, round(stat,2), round(p,4), round(d,2)])
infer = pd.DataFrame(tests, columns=["Level","Criterion","N","Test","Stat","p","Cohen_d"])
infer.to_csv(OUT/"inferential_stats.csv", index=False)

# ------------------------------------------------------------------
# 7. 3-way repeated-measures ANOVA (Agent × Level × Criterion)
# ------------------------------------------------------------------
# Since pingouin doesn't support 3-way ANOVA, we'll do separate 2-way ANOVAs
aov_results = []

# Agent × Level ANOVA
aov1 = pg.rm_anova(data=long, dv="Score",
                   within=["Agent","Level"],
                   subject="ID", detailed=True)
aov1["Analysis"] = "Agent × Level"
aov_results.append(aov1)

# Agent × Criterion ANOVA
aov2 = pg.rm_anova(data=long, dv="Score",
                   within=["Agent","Criterion"],
                   subject="ID", detailed=True)
aov2["Analysis"] = "Agent × Criterion"
aov_results.append(aov2)

# Level × Criterion ANOVA
aov3 = pg.rm_anova(data=long, dv="Score",
                   within=["Level","Criterion"],
                   subject="ID", detailed=True)
aov3["Analysis"] = "Level × Criterion"
aov_results.append(aov3)

# Combine results
aov = pd.concat(aov_results, ignore_index=True)
aov_file = OUT/"rm_anova.csv"; aov.round(4).to_csv(aov_file, index=False)

# ------------------------------------------------------------------
# 8. pretty version of Comparative Performance Matrix --------------
pivot = (desc.pivot_table(index="Criterion",
                          columns=["Level","Agent"],
                          values="Label", aggfunc="first")
              .reindex(index=sorted(COLUMN_MAP["Operational"].keys())))
print("\n==== Comparative Performance Matrix (labels) ====\n")
print(pivot.to_string())

print("\nFiles saved:")
for f in ["summary_table.csv","inferential_stats.csv","rm_anova.csv"]:
    print("  •", OUT/f)
